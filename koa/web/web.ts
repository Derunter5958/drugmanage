...
class UserListClass implements UserListInterface<number,string>{ // 泛型<写动态的类型>
    ...
    UserListMethod(){
        routerWWW.get('/userlist',async function(ctx:any,next:Function){
            // 调用controller层
            // 返回所有的数据
    ...
    SigleUserListMethod(){
        routerWWW.get('/userlist/:id',async function(ctx:any,next:Function){
            var { id } = ctx.params;

            // var Params = new PramsEncap();  // 传递过来的参数：封装到类里面 
            // Params.set(3);

            var SigleUserList = await Controller.ControllerSigleUserList(id);
            ...
            ctx.body = 'userlist单条接口';
            await next();
        })
    }
}

let UserlistObject = new UserListClass('userlist类');
UserlistObject.UserListMethod();
UserlistObject.SigleUserListMethod();

//导出router

module.exports = routerWWW;